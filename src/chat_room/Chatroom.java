...
//	public ArrayList<String> newlistitem;
	...
	public Chatroom(String user, String rname, Boolean type){
		account = user;
		roomid = rname;
		groupstate = type;
		...
		public void run() {
			...
			try {
					...
					msgcut tool = new msgcut();
						while(true) {
							Msg = input.readLine();
							if(Msg.contains("ResponseType:R04")) {
									List<String> history= tool.get_Log(Msg);
									System.out.println(history);
									Iterator<String> it =  history.iterator();
									while(it.hasNext()) {
										String logit = it.next();
							...
							else if(Msg.contains("ResponseType:C")) {
